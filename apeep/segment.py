...
def segmentation_threshold(img, method="auto", threshold=0.5, var_limit=0.0015):
    ...
    if method == "static":
        ...
        # precise and faster
        crop = round(img_small.shape[0]/4)
        img_center_small = img_small[crop:3*crop,:] # central band = more noise, fewer artifacts
        ...
        if method == "percentile":
            ...
            gray_threshold_center = np.percentile(img_center_small, (threshold))
            gray_median = np.median(img_small)
            gray_median_center = np.median(img_center_small)
        elif method == "otsu":
            gray_threshold = skimage.filters.threshold_otsu(img_small)
        else:
            raise ValueError("unknown `method` argument")
    
    return gray_threshold, gray_threshold_center, gray_median, gray_median_center