...
def frame_list(dir):
    """
    List the frames, their absolute start line and their associated avi file in a directory. 
    
    Args:
        dir (str): path to input directory containing .avi files
    
    Yields:
         frames (list): list of frames and their associated avi file
    """
    # get general logger
    log = logging.getLogger()
    ...
    for avi in all_avi:
        ...
        for frame in v.decode(video=0):
            ...
            # add frame number and its avi file to list of frames
            frames.append([avi.split('/')[-1], i_f])
            
            # increase frame index
            i_f = i_f + 1
    
    # convert to dataframe
    frames = pd.DataFrame(frames, columns=['avi_file', 'frame'])
    
    # compute line start number for each frame (each frame contains 2048 lines)
    frames = frames.assign(frame_line_start = frames.index*2048)
