...
def resolve_overlaps(img_preds, frames_props, overlap_threshold=0.2):
    """
    Resolve semantic prediction overlaps with following steps:
    ...
    for t in pot_overlaps.index:
        ...
        if (frame_col_i == frame_col_j) & (frame_row_i < frame_row_j):
            ...
            over_v = int(row1_i - row0_j) # vertical overlap
        ...
        if (frame_col_i > frame_col_j) & (frame_row_i < frame_row_j):
            ...
            # Compute horizontal and vertical overlap
            col0_i = frames_props.loc[(frames_props['frame_row'] == frame_row_i) & (frames_props['frame_col'] == frame_col_i), 'col0'].values[0] # left column of frame 1
            col1_j = frames_props.loc[(frames_props['frame_row'] == frame_row_j) & (frames_props['frame_col'] == frame_col_j), 'col1'].values[0] # right column of frame 2
...
def merge_masks(semantic_mask, regular_mask):
    ...
    Returns:
        ndarray: labelled merged image mask 
    """
